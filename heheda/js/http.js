        ...
        xhr.onload = function () {
            ...
            if (result.code == 200) {
                ...
                xhr.onload = function () {
                    ...
                    console.log(result);
                    let ipaddress = document.getElementById("ipaddress");
                    ipaddress.innerText = "您的IP：" + result.ip;
                    let ipnode = document.getElementById("ipnode");
                    ipnode.innerText = "您的节点：" + result.data.owner;
                    let ipposition = document.getElementById("ipposition");
                    ipposition.innerText = "您的位置：" + result.data.continent + result.data.country + result.data.prov + result.data.city + result.data.district;
                }
            }
        }
    }
